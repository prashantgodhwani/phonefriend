...
class AdminController extends Controller
{
    ...
    public function getPhonesBetweenDates(Request $request){
        ...
        $orders = DB::table('order_devices')->select('order_devices.phone_id', DB::raw("count('order_devices.order_id') as sales"))->leftJoin('orders','orders.id','=','order_devices.order_id')->whereBetween(DB::raw('date(orders.created_at)'), [Carbon::createFromFormat('Y-m-d', $start)->toDateString(), Carbon::createFromFormat('Y-m-d', $end)->toDateString()])->where('order_devices.phone_id',$request->phoneId)->groupBy('order_devices.phone_id')->orderBy('sales', 'DESC')->get();
        ...
        $codCancelled = DB::table('order_devices')->select('order_devices.phone_id', DB::raw("count('order_devices.order_id') as sales"))->leftJoin('orders','orders.id','=','order_devices.order_id')->whereBetween(DB::raw('date(orders.created_at)'), [Carbon::createFromFormat('Y-m-d', $start)->toDateString(), Carbon::createFromFormat('Y-m-d', $end)->toDateString()])->where('orders.payment_mode','cod')->where('order_devices.phone_id',$request->phoneId)->where('order_status', 'Cancelled')->groupBy('order_devices.phone_id')->orderBy('sales', 'DESC')->get();
